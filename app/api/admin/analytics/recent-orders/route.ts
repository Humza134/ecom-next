import { getRecentOrdersService } from "@/app/services/admin-analytics";
...
export async function GET(req: NextRequest) {
    try {
        ...
        // Call Heavy Service 1
        const response = await getRecentOrdersService();
        let status = 200;
        if(!response.success) {
            status = response.error?.code === "INTERNAL_ERROR" ? 500 : 400;
        }
        return NextResponse.json<ApiResponse<RecentOrder[]>>(response, { status });
    } catch (error) {
       return NextResponse.json<ApiResponse<null>>({
            success: false,
